...
| **対応方針の記録** | 脅威ごとにリスク評価と対応方針（低減／受容／移転／回避）を残し、エクスポートする |
...
> このページは**初回起動時のサンプル図**で操作を説明します。
> 次章以降は、[はじめての脅威モデル](first-threat-model.ja.md)で自分で描く
> AI チャットボットの図を、ガイド全体を通して使います。
...
カードの下部では、対策の実装状況・リスク評価・リスク対応方針を記録できます。
