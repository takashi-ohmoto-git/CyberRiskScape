...
![Analytics の DREAD 評価](../assets/guide/threat-panel/06-analytics-dread.png)

画面は 3 つに分かれています。

- **左** — コンポーネント別の脅威ツリー（要素ごとに最大 severity と件数）
- **中央** — 対策の一覧（実装状況の未設定件数もここで分かる）
- **右** — 選択した脅威の詳細と **DREAD 評価**フォーム

DREAD は 5 項目をそれぞれ **1（低）／2（中）／3（高）** の 3 段階で評価します。

| 項目 | 見るもの |
|---|---|
| D: 損害（Damage） | 成立したときの被害の大きさ |
| R: 再現性（Reproducibility） | どれだけ安定して再現するか |
| E: 攻撃容易性（Exploitability） | 攻撃にどれだけの技術・手間が要るか |
| A: 影響範囲（Affected Users） | 何人・どの範囲に及ぶか |
| D: 発見容易性（Discoverability） | 攻撃者がどれだけ見つけやすいか |

各項目には判断基準が添えられているので、評価者による振れ幅を抑えられます。
合計は **5〜15** で、次のようにランクへ変換されます。
...
上の例では、ルール由来が High の脅威が、合計 10 点で Medium になっています
（元の値は括弧内に残ります）。

画面上部のフィルタ（全件／High 以上／緩和策あり／緩和策なし）と検索欄で、
評価する対象を絞り込めます。**「High 以上」から順に潰していく**のが現実的な進め方です。
...
メニューに「表示中の脅威一覧（L1 / ALL・38 件）を出力」と出ているとおり、
