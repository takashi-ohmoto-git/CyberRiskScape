...
要求（往路）だけでなく、**応答（復路）のデータフローも引きます**。
脅威の判定はエッジの向きを見るため、往路だけの図では
「検索結果が LLM に入る」ことを前提とする脅威が出てきません（詳細は §5.4）。
...
まず要求（往路）のデータフローを 4 本引きます。
手順は「始点を選択 → **CREATE LINK** → 終点をクリック」です。
...
続けて、**応答（復路）を逆向きに 4 本**引きます。

5. データストア → ベクターDB / RAG
6. ベクターDB / RAG → LLMモデル
7. LLMモデル → Front-end Server
8. Front-end Server → ユーザー

同じ 2 点を結ぶ線は、自動的に左右に振り分けて描かれます。

![往復 8 本の接続を引いた状態](../assets/guide/chatbot/05-connections.png)
...
復路の Front-end Server → ユーザーも同じ経路を通るので、同様に設定します。

### 5.4 なぜ復路まで引くのか

脅威ルールの接続条件は、**エッジの source → target** を見ます。
「順方向 / 逆方向 / 双方向」の設定は矢印の描き方を変えるだけで、判定には使われません。

たとえば **間接プロンプトインジェクション**は、
「LLM に対して RAG・データストア等からの inbound エッジがある」ことが発火条件です。
往路（LLM → RAG）しか描いていない図では、この脅威は出てきません。
復路（RAG → LLM）を引いて初めて、**検索結果に仕込まれた指令**という
RAG 構成の中心的な脅威が現れます。

この図では、復路 4 本を足したことで検出数が **43 件 → 48 件**に増えました。

なお、ベクターDB / RAG → LLMモデル の線には
**Semantic（エッジ意味論）に `rag_retrieval`** を設定しておくと、
その経路が検索結果の取得であることを明示できます。
...
完成すると、右ペインに脅威が並びます。この例では 48 件でした
（件数は属性の設定内容によって変わります）。
...
3. **優先順位を付ける** — リスク評価と対応方針（低減／受容／移転／回避）を記録します。
...
ここまでの図は、[テンプレート](templates.ja.md)として書き出しておけば次回から再利用できます。
同じ構成のテンプレートは [`templates/ai-chatbot.ja.json`](templates/ai-chatbot.ja.json) から
ダウンロードできます。
...
- 図を再利用する — [テンプレートを作る・使う](templates.ja.md)
