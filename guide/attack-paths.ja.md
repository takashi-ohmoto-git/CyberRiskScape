...
このページは [Analytics でリスクを評価する](analytics-assessment.ja.md) の続きです。
記録したリスク評価と対策実装状況がそのまま分析に効いてくるので、先にそちらを読むことをお勧めします。
図は [はじめての脅威モデル](first-threat-model.ja.md) で作った AI チャットボット構成を使います。
...
Front-end Server（公開面からの直接攻撃）の 2 本を引いています。
...
- **経路 2（チャネル組合せ 24）** — 到達経路は 2 本（標的はデータストア）。
  括弧内は並行するデータフローの組合せ数で、**参考値**です。
  往復のフローを引いてあるため組合せは 24 通りになりますが、**経路としては 2 本**です。
  同じノード列を通る経路は、通信路が何本あっても **1 経路**として数えます。
- **最小コスト 12** — 最も楽な経路のコスト。小さいほど攻撃者にとって容易です。
- **リスク未評価のため…** — リスク評価を 1 件も付けていないときに出る注記（後述）。
...
| 表示 | 意味 |
|---|---|
| 難易度 n | 攻撃容易性（Exploitability）の評価から算出した値（小さいほど通りやすい） |
| 暫定 n | リスク未評価のため、severity から仮に置いた値 |
| 未評価 | 判断材料がない |

ノードの間にある小さなチップは**論理ホップ**です。同じ 2 点を結ぶ並行チャネルが複数ある場合は
`×2` のようにまとめて表示されます（この図は往復のフローを引いてあるので、
どのホップも `×2` になります）。**攻撃者は最も通りやすいチャネルを選べる**ため、
...
上の例では、Front-end Server・LLM モデル・ベクターDB / RAG がいずれも
「2 経路中 2 本が通過」です。つまり**ここに対策を置けば、両方の経路に同時に効く**ことが分かります。
ユーザー経由の経路にだけ対策を打っても、公開面からの直接攻撃は止まりません。
...
`C6 → C2 → C3 → C4 → C5`（コスト 12）が最も楽な経路で、
ユーザーを経由する経路（コスト 14）より攻撃者にとって好ましいと読めます。
...
| 条件 | 難易度 |
|---|---|
| リスク評価済み | `4 − max(Exploitability)` |
| 脅威はあるがリスク未評価 | severity から暫定値（Critical・High → 1、Medium → 2、Low → 3） |
| 脅威なし | 中立の 2 |
...
ここでいう**被覆は、[Analytics](analytics-assessment.ja.md) で記録した「対策実装状況」から決まります**。
...
- 脅威カードの読み方 — [脅威パネルの読み方](reading-threats.ja.md)
